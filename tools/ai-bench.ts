// Tier-1 AI benchmark (owner: src/ai). `npm run bench:ai -- --games N --seed S [--difficulty easy|normal]`
// Plays UtilityDecider vs RandomDecider on the real Combat Patrol rosters through the public engine API
// (createGame/legalActions/step/view), alternating which faction and which seat the AI takes, cycling missions
// cp-01..cp-06. Prints games, AI wins, draws, mean VP each side, and mean/p95 ms per AI decision.
...
// factions from both seats rather than always being (faction 0, seat A) / (faction 1, seat B).
function makeSetup(bundle: DataBundle, gameIndex: number, aiSeat: PlayerId, dataVersion: string): GameSetup {
  ...
  const aiPatrolIdx = Math.floor(gameIndex / 2) % 2
  const aiPatrol = patrols[aiPatrolIdx]
  const oppPatrol = patrols[(aiPatrolIdx + 1) % 2]
...
interface GameOutcome { aiSeat: PlayerId; winner: PlayerId | 'draw' | null; vpAi: number; vpRandom: number; decisionMs: number[]; rejections: number; unfinished: boolean }

async function playOne(bundle: DataBundle, seed: string, gameIndex: number, dataVersion: string, difficulty: Difficulty): Promise<GameOutcome> {
  const aiSeat: PlayerId = gameIndex % 2 === 0 ? 'A' : 'B'
  const setup = makeSetup(bundle, gameIndex, aiSeat, dataVersion)
...
async function main(): Promise<void> {
  ...
  for (let g = 0; g < games; g++) {
    const t0 = Date.now()
    const outcome = await playOne(bundle, seed, g, DATA_VERSION, difficulty)
    rejections += outcome.rejections
    if (outcome.unfinished) unfinished++
    vpAiTotal += outcome.vpAi
    vpRandomTotal += outcome.vpRandom
    allMs.push(...outcome.decisionMs)
    if (outcome.winner === 'draw') draws++
    else if (outcome.winner === outcome.aiSeat) aiWins++
    else if (outcome.winner !== null) randomWins++
    if (verbose) {
      const meanMs = outcome.decisionMs.length ? outcome.decisionMs.reduce((a, b) => a + b, 0) / outcome.decisionMs.length : 0
      console.log(`game ${g} aiSeat=${outcome.aiSeat}: ${outcome.winner ?? 'unfinished'} VP ai=${outcome.vpAi} random=${outcome.vpRandom}, mean ai decision ${meanMs.toFixed(1)}ms, ${Date.now() - t0}ms${outcome.rejections ? `, ${outcome.rejections} rejection(s)` : ''}`)
    }
  }
  ...
  console.log(`ai-bench: ${games} games, seed ${seed}, difficulty ${difficulty}, ${((Date.now() - started) / 1000).toFixed(1)}s`)
