...
- Phase modules `src/engine/phases/{command,movement,shooting,charge,fight}.ts`, `attack.ts`, `weapons.ts`, `transports.ts`, `setup.ts`, `objectives.ts`, `missions.ts`, `tools/sim` — still stubs per `src/engine/phases/README.md` ownership table (W1-C/D/E/G); they build on the services landed above
...
- Next up is W1-C/D/E: the phase modules (movement, shooting, charge, fight, command) are still stubs and can now build on real `terrain`/`los`/`hooks`/`effects`/`stratagems`/`leaders`/`enhancements` services (see the M1 foundations table above and `src/engine/phases/README.md` §4 for what each service already provides). `services.stratagems.openWindow`/`ctx.window` and `code-hooks.ts`'s `pendingReactions`/`consumeReaction` queue are ready for Fire Overwatch / Heroic Intervention / Rapid Ingress / Counter-offensive once a phase module calls them.
- `terrain.canEndAt` and `los.ts`'s model-blocker test were adversarially wrong on first pass (see "Fixed during verification" above) — worth a similar adversarial pass on the phase modules before trusting them past their own happy-path tests.
